...
const RSVP_DEADLINE=new Date("2026-10-26T00:00:00-06:00");
...
const guestNameField=document.querySelector("#guestName");
const guestNameLabel=document.querySelector("#guestNameLabel");
const submitRsvp=document.querySelector("#submitRsvp");
const formStatus=document.querySelector("#formStatus");
const rsvpClosedMessage=document.querySelector("#rsvpClosedMessage");
function isRsvpClosed(){return Date.now()>=RSVP_DEADLINE.getTime()}
function openRsvpDialog(){const closed=isRsvpClosed();formStatus.className="form-status";formStatus.textContent="";rsvpForm.hidden=closed;rsvpClosedMessage.hidden=!closed;rsvpDialog.showModal();document.body.classList.add("dialog-open");setTimeout(()=>(closed?document.querySelector("#closeRsvp"):attendance).focus(),0)}
...
attendance.addEventListener("change",()=>{const attending=attendance.value==="si";guestCount.disabled=!attending;guestCount.required=attending;guestCount.value=attending?Math.max(1,Number(guestCount.value)||1):0;guestNameLabel.textContent=attending?"Nombres completos de quienes asistirán":"Nombre completo de quien responde";guestNameField.placeholder=attending?"Escribe un nombre por línea":"Escribe tu nombre completo"});
...
rsvpForm.addEventListener("submit",async event=>{event.preventDefault();if(isRsvpClosed()){rsvpForm.hidden=true;rsvpClosedMessage.hidden=false;return}if(rsvpForm.website.value)return;const data=new FormData(rsvpForm);const attending=data.get("attendance")==="si";const payload={guest_name:String(data.get("guest_name")||"").trim().slice(0,600),attendance:attending?"si":"no",guest_count:attending?Math.min(20,Math.max(1,Number(data.get("guest_count"))||1)):0,phone:String(data.get("phone")||"").trim()||null,message:String(data.get("message")||"").trim()||null,invitation:String(data.get("invitation")||"Invitación general").slice(0,160),user_agent:navigator.userAgent.slice(0,500)};submitRsvp.disabled=true;submitRsvp.textContent="Enviando…";showFormStatus("Enviando tu respuesta…");try{const response=await fetch(RSVP_ENDPOINT,{method:"POST",headers:{apikey:SUPABASE_PUBLISHABLE_KEY,"Content-Type":"application/json",Prefer:"return=minimal"},body:JSON.stringify(payload)});if(!response.ok)throw new Error(`RSVP request failed with status ${response.status}`);rsvpForm.reset();attendance.dispatchEvent(new Event("change"));invitationField.value=guestName?guestName.slice(0,160):"Invitación general";showFormStatus("¡Confirmación registrada! Gracias, tu respuesta quedó guardada correctamente.")}catch(error){console.error("No se pudo registrar la confirmación",error);showFormStatus("No se pudo registrar tu respuesta. Revisa tu conexión e intenta nuevamente.",true)}finally{submitRsvp.disabled=false;submitRsvp.textContent="Enviar confirmación"}});
